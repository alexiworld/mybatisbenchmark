...
@RestController
@RequestMapping("/rest/v1/dynatemplate/todo")
public class TodoDynaTemplateController {
    ...
    public Todo saveTodo(@RequestBody Todo todo) {
        todoDao.insertUsingTemplate(todo);
        return todo;
    }
    ...
    public Todo updateTodo(@PathVariable Integer id, @RequestBody Todo todo) {
        todo.setTodoId(id);
        todoDao.updateUsingTemplate(todo);
        return todo;
    }

    @GetMapping("/")
    public List<Todo> getTodos() {
        List<Todo> todosRet = todoDao.findAllUsingTemplate();
        return todosRet;
    }

    @GetMapping("/{id}")
    public Todo getTodo(@PathVariable("id") Integer id) {
        Todo todoRet = todoDao.findByIdUsingTemplate(id);
        return todoRet;
    }

    @DeleteMapping("/{id}")
    public void deleteTodo(@PathVariable("id") Integer id) {
        todoDao.deleteUsingTemplate(id);
    }
